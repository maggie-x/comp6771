...
SCENARIO("Instantiating a new graph") {
GIVEN("A vector of nodes") {
std::vector<std::string> v{"hello", "how", "are", "you"};
std::sort(v.begin(), v.end());

gdwg::Graph<std::string, double> g{v.cbegin(), v.cend()};

WHEN("A new graph is instantiated from the vector") {
THEN("The graph should be populated with expected nodes") {
auto nodes = g.GetNodes();
//REQUIRE(std::equal(v.begin(), v.end(), nodes.begin()));
}
}
}

GIVEN("A vector of nodes and edges") {
std::vector<std::tuple<std::string, std::string, double>> v{
        {"are", "you", 1.0},
        {"hello", "how", 1.0},
        {"how", "are", 1.0},
        {"you", "hello", 1.0},
...
WHEN("A new graph is instantiated from the vector") {
THEN("The graph should be populated with expected nodes and edges") {
//REQUIRE(std::equal(v.begin(), v.end(), g.begin()));
}
}
}
...
gdwg::Graph<std::string, double> g{"hello", "how", "are", "you"};

WHEN("A new graph is instantiated from a initializer list") {
THEN("The graph should be populated with expected nodes") {
auto nodes = g.GetNodes();
//REQUIRE(std::equal(v.begin(), v.end(), nodes.begin()));
}
...
GIVEN("A valid graph to copy from") {
std::vector<std::tuple<std::string, std::string, double>> v{
        {"are", "you", 1.0},
        {"hello", "how", 1.0},
        {"how", "are", 1.0},
...
WHEN("Creating a copy of the graph") {
gdwg::Graph<std::string, double> cpy{g};

THEN("An identical copy should be constructed") { REQUIRE(cpy == g); }

WHEN("Altering the copy") {
cpy.InsertNode("dummy");

THEN("The copy should not reflect the original") { REQUIRE(cpy != g); }
}
}
}

GIVEN("A valid graph to move from") {
std::vector<std::tuple<std::string, std::string, double>> v{
        {"are", "you", 1.0},
        {"hello", "how", 1.0},
        {"how", "are", 1.0},
...
WHEN("Moving from the graph") {
gdwg::Graph<std::string, double> mov{std::move(g)};

THEN("The moved graph should be equal to a copy of the original") { REQUIRE(mov == cpy); }

THEN("The moved-from graph should be equivalent to an empty graph") {
...
const gdwg::Graph<std::string, double> g{v.cbegin(), v.cend()};

WHEN("Copy assigning the graph to a new graph") {
gdwg::Graph<std::string, double> cpy = g;

THEN("The new graph should be the same as the old graph") { REQUIRE(g == cpy); }

WHEN("Modifying the copy") {
cpy.InsertNode("dummy");

THEN("The copy should not reflect the original") { REQUIRE(g != cpy); }
}
}
}
...
WHEN("Move assigning the graph to a new graph") {
gdwg::Graph<std::string, double> mov = std::move(g);

THEN("The moved graph should be the same as a copy of the old graph") { REQUIRE(mov == cpy); }
...
WHEN("Checking for an existent node") {
THEN("IsNode should return true") { REQUIRE(g.IsNode("how")); }
}

WHEN("Checking for a non-existent node") {
THEN("IsNode should return false") { REQUIRE(!g.IsNode("dummy")); }
}

WHEN("Checking for an existent edge") {
THEN("IsEdge should return true") { REQUIRE(g.IsConnected("are", "you")); }
}

WHEN("Checking for a non-existent edge") {
THEN("IsEdge should return false") { REQUIRE(!g.IsConnected("how", "you")); }
}

WHEN("Checking for an non-existent edge with non-existent nodes") {
THEN("IsEdge should throw an exception") {
REQUIRE_THROWS_WITH(
        g.IsConnected("a", "b"),
"Cannot call Graph::IsConnected if src or dst node don't exist in the graph");
}
...
SCENARIO("Getting members and edges") {
GIVEN("A graph with edges and nodes") {
std::vector<std::tuple<std::string, std::string, double>> v{
        {"are", "you", 1.0}, {"hello", "how", 1.0}, {"hello", "how", 5.0},
        {"how", "are", 1.0}, {"you", "hello", 1.0},
};
...
THEN("Vector of nodes should match expected") {
std::vector<std::string> expected{"are", "hello", "how", "you"};

//REQUIRE(std::equal(expected.begin(), expected.end(), nodes.begin()));
}
}

WHEN("Getting a list of connections from a node") {
auto connected = g.GetConnected("hello");

THEN("Vector of connected nodes should match expected") {
std::vector<std::string> expected{"how"};
REQUIRE(connected == expected);
}
}

WHEN("Getting a list of weights between two nodes") {
auto weights = g.GetWeights("hello", "how");

THEN("Vector of weights should match expected") {
std::vector<double> expected = {1.0, 5.0};

//REQUIRE(std::equal(expected.begin(), expected.end(), weights.begin()));
}
}

WHEN("Finding an edge") {
AND_WHEN("The edge exists") {
auto edge = g.find("how", "are", 1.0);

THEN("The edge should not point to .cend()") { REQUIRE(edge != g.cend()); }

AND_THEN("The edge should return correct data") {
std::tuple<std::string, std::string, double> expected = {"how", "are", 1.0};

REQUIRE(expected == *edge);
}
}

AND_WHEN("The edge doesn't exist") {
auto edge = g.find("how", "wow", 1.0);

THEN("The edge should return .cend()") { REQUIRE(edge == g.cend()); }
}
}
}
...
WHEN("Getting a list of nodes") {
THEN("The vector of nodes should be empty") { REQUIRE(!g.GetNodes().size()); }
}

WHEN("Getting a list of connections from an non-existent node") {
THEN("An exception should be thrown") {
REQUIRE_THROWS_WITH(g.GetConnected("unknown"),
"Cannot call Graph::GetConnected if src doesn't exist in the graph");
}
}

WHEN("Getting a list of weights from a non-existent src") {
THEN("An exception should be thrown") {
REQUIRE_THROWS_WITH(
        g.GetWeights("unknown", "how"),
"Cannot call Graph::GetWeights if src or dst node don't exist in the graph");
}
}

WHEN("Getting a list of weights with a non-existent dst") {
THEN("An exception should be thrown") {
REQUIRE_THROWS_WITH(
        g.GetWeights("how", "unknown"),
"Cannot call Graph::GetWeights if src or dst node don't exist in the graph");
}
}
}
...
WHEN("Erasing an edge") {
AND_GIVEN("A valid src, dst and weight combination") {
THEN("Erase should return true") {
REQUIRE(g.erase("are", "you", 1.0));

AND_THEN("Edge should no longer exist in graph") {
REQUIRE(g.find("are", "you", 1.0) == g.cend());
}
}
}

AND_GIVEN("An invalid src, dst and weight combination") {
THEN("Erase should return false") { REQUIRE(!g.erase("are", "poo", 1.0)); }
}

AND_GIVEN("A valid const_iterator") {
const auto it = g.find("are", "you", 1.0);

THEN("Erase should return iterator after value") {
auto cpy{it};
cpy++;

REQUIRE(g.erase(it) == cpy);

AND_THEN("Edge should be erased from the graph") {
REQUIRE(g.find("are", "you", 1.0) == g.cend());
}
}
}

AND_GIVEN("An invalid const_iterator") {
const auto it = g.cend();

THEN(".cend() should be returned") { REQUIRE(g.erase(it) == g.cend()); }
}
}
}
}
...
// XXX: Insert these nodes to try to mess up the iterator with unlinked nodes
g.InsertNode("Bob");
g.InsertNode("Ted");

WHEN("Iterating over the graph") {
THEN("The contents of the iterator should match expected") {
//REQUIRE(std::equal(v.begin(), v.end(), g.begin()));
}
}

WHEN("Reverse iterating over the graph") {
THEN("The contents of the iterator should match expected") {
//REQUIRE(std::equal(v.rbegin(), v.rend(), g.rbegin()));
}
}

WHEN("Incrementing a forward iterator") {
AND_WHEN("Using pre-increment") {
THEN("The contents of the iterator should match the expected") {
auto it = g.cbegin();

std::tuple<std::string, std::string, double> expected = {"hello", "how", 1.0};

REQUIRE(*++it == expected);
}
}

AND_WHEN("Using post-increment") {
THEN("The contents of the iterator should match the expected") {
auto it = g.cbegin();

std::tuple<std::string, std::string, double> expected_before = {"are", "you", 1.0};
std::tuple<std::string, std::string, double> expected_after = {"hello", "how", 1.0};

REQUIRE(*it++ == expected_before);
REQUIRE(*it == expected_after);
}
}
}

WHEN("Incrementing a reverse iterator") {
AND_WHEN("Using pre-decrement") {
THEN("The contents of the iterator should match the expected") {
auto it = g.find("you", "hello", 1.0);
...
std::tuple<std::string, std::string, double> expected_before = {"you", "hello", 1.0};
std::tuple<std::string, std::string, double> expected_after = {"how", "are", 1.0};
...
const gdwg::Graph<std::string, double> g1{v.begin(), v.end()};
const gdwg::Graph<std::string, double> g2{v.begin(), v.end()};

WHEN("Comparing for equality") {
THEN("Comparison should return true") { REQUIRE(g1 == g2); }
}

WHEN("Comparing for inequality") {
THEN("Comparison should return false") { REQUIRE(!(g1 != g2)); }
}
}

GIVEN("Two unequal graphs") {
std::vector<std::tuple<std::string, std::string, double>> v{
        {"are", "you", 1.0}, {"hello", "how", 1.0}, {"hello", "how", 5.0},
        {"how", "are", 1.0}, {"you", "hello", 1.0},
};
...
SCENARIO("Printing out the graph") {
GIVEN("A graph with fully-connected nodes and edges") {
std::vector<std::tuple<std::string, std::string, double>> v{
        {"are", "you", 1.0}, {"hello", "how", 1.0}, {"hello", "how", 5.0},
        {"how", "are", 1.0}, {"you", "hello", 1.0},
};

const gdwg::Graph<std::string, double> g{v.begin(), v.end()};

WHEN("Printing out the graph") {
std::string expected_output = "are (\n"
                              "  you | 1\n"
                              ")\n"
                              "hello (\n"
                              "  how | 1\n"
...
GIVEN("A graph with not connected nodes and edges") {
std::vector<std::tuple<std::string, std::string, double>> v{
        {"are", "you", 1.0}, {"hello", "how", 1.0}, {"hello", "how", 5.0},
        {"how", "are", 1.0}, {"you", "hello", 1.0},
};
