...
class Graph {
 public:
  ...
  struct Node {
    ...
    bool InsertOutgoing(std::shared_ptr<N> dst,
                        const E& weight) {  // pass in shared ptr after finding node in set of nodes
    ...
    // return iterator to an edge from this node to dst with a specified weight. If it doesn't
    // exist, return .end()
  ...
  class const_iterator {
   public:
    ...
    explicit const_iterator(typename std::set<Node>::const_iterator nodes_pos,
                            typename std::set<Edge>::const_iterator edges_pos)
      : node_it_{nodes_pos}, edge_it_{edges_pos} {};
  ...
  // All instances of node oldData in the graph are replaced with instances of newData. Throws
  // exception if either node cannot be found in the graph
  ...
  // returns true if an edge from src to dst exists in the graph. Throws exception if src or dst not
  // in graph
  ...
  // increasing order of nodes. Throws exception if src not in graph
  std::vector<N> GetConnected(const N& src) const;

  // returns vector of edge weights from src to dst, sorted by increasing order of weights. Throws
  // exception if src or dst not in grpah
